...
function renderTable(rows){
  ...
  for(const r of filtered){
    ...
    const displayState=isBan?"Banned`nリーダーボードから完全消失。BANの可能性が高い状態です。<span class="badge" style="background:#2a0808;color:#ff5555;border-color:#7a1a1a;margin:0 5px 0 2px">笵・BAN</span>'
      :isNameChange
...
function renderBanList(rows){
  ...
  let html="";
  if(Banned`nリーダーボードから完全消失。BANの可能性が高い状態です。</div>`+Banned`nリーダーボードから完全消失。BANの可能性が高い状態です。</div>`+nameChanged.map(itemHtml).join("");
...
function renderLogTimeline(){
  ...
  // 蜃｡萓・  const legendStates=[["OFFLINE","#8ea0b7","Offline"],["LOBBY","#5b9cf6","Lobby"],["IN_MATCH","#39d98a","In Match"],["IN_TOURNAMENT_DEEP","#c77dff","Final/Tournament"],["NOT_FOUND","#ff9944","Missing"],["Banned`nリーダーボードから完全消失。BANの可能性が高い状態です。<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;">`;
