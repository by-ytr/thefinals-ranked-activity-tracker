...
let lastSnapshotSync=0; // 蜈ｱ譛臥憾諷九・鬮倬溷酔譛・const lastSubmittedSnapshotSig=new Map(); // name -> signature
...
async function pollOnce(names,settings){
  ...
  saveSnapshots(snapshots);
  // 蜈ｱ譛牙ｯｾ雎｡・・ommunity 逋ｻ骭ｲ貂医∩・峨・迥ｶ諷九・縲√ち繝悶↓髢｢菫ゅ↑縺上ヰ繝・け繧ｨ繝ｳ繝峨∈騾∽ｿ｡
...
function doStart(){
  ...
  (function schedulePoll(){
    // /community 縺ｯ 120 遘偵＃縺ｨ縲・snapshots 縺ｯ 3 遘偵＃縺ｨ縺ｫ蜷梧悄
    const now=Date.now();
    const _gUrl=effectiveGlobalUrl(settings);
