  ...
  try{
    ...
    for(const [key,remSnap] of Object.entries(remote)){
      ...
      const merged={
        ...locSnap,
        ...remSnap,
        // 謇句虚驕ｭ驕・・閾ｪ遶ｯ譛ｫ縺ｮ謫堺ｽ懊ｒ蜆ｪ蜈・        ...(locSnap.manualEvent?{manualEvent:locSnap.manualEvent}:{}),
      };
      // lastChangeAt 縺檎┌縺・ョ繝ｼ繧ｿ縺ｧ current state 縺瑚誠縺｡繧九・繧帝亟縺・      if(!merged.lastChangeAt&&remSnap.lastChangeAt)merged.lastChangeAt=remSnap.lastChangeAt;
...
// 蜆ｪ蜈磯・ｽ・ admin hash 竊・繝ｭ繧ｰ繧､繝ｳ荳ｭ allowed user 縺ｮ hash 竊・辟｡縺暦ｼ域悴繝ｭ繧ｰ繧､繝ｳ・・function getWriteHeaders(){
  ...
  // admin 縺ｨ縺励※險ｭ螳壽ｸ医∩縺ｪ繧・admin hash 繧剃ｽｿ縺・  if(auth.adminPasswordHash){
    h["X-Write-Key"]=auth.adminPasswordHash;
    return h;
  }
  // allowed user 縺ｨ縺励※繝ｭ繧ｰ繧､繝ｳ荳ｭ縺ｪ繧峨√◎縺ｮ user 縺ｮ hash 繧剃ｽｿ縺・  // auth.allowedUsers 縺ｯ fetchAuthConfig() 縺ｧ繝舌ャ繧ｯ繧ｨ繝ｳ繝峨°繧牙叙蠕玲ｸ医∩ ({id, passwordHash}[])
...
let _backendAllowedUsers=null; // null = 譛ｪ繝輔ぉ繝・メ or globalUrl 縺ｪ縺・async function fetchAuthConfig(globalUrl){
  ...
  try{
    ...
    if(Array.isArray(d.allowedUsers)){
      ...
      // 蠕ｩ蜈・＠縺溘そ繝・す繝ｧ繝ｳ縺ｮ繝ｦ繝ｼ繧ｶ繝ｼ縺悟炎髯､縺輔ｌ縺ｦ縺・◆繧芽・蜍輔Ο繧ｰ繧｢繧ｦ繝・      if(currentUser&&d.allowedUsers.length>0&&!d.allowedUsers.find(u=>u.id.toLowerCase()===currentUser.id.toLowerCase())){
        setCurrentUser(null);
      }
      updateLoginStatus(); // 繝舌ャ繧ｯ繧ｨ繝ｳ繝峨Μ繧ｹ繝医〒迥ｶ諷九ｒ譖ｴ譁ｰ
    }
  }catch{}
}
...
async function pollOnce(names,settings){
  ...
  saveSnapshots(snapshots);
  // 繧ｰ繝ｭ繝ｼ繝舌Ν繝｢繝ｼ繝・ 繧ｹ繝翫ャ繝励す繝ｧ繝・ヨ繧偵ヰ繝・け繧ｨ繝ｳ繝峨↓騾∽ｿ｡・井ｻ悶Θ繝ｼ繧ｶ繝ｼ縺ｨ蜈ｱ譛会ｼ・  if(viewMode==="global"){
